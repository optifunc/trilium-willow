...
Open [the design](index.html) directly in a browser. No build, server, account, or network access is required. The toolbar is 41px high and the status bar is 30px high. [Captured previews](previews/) include light/dark × full/split layouts, incoming changes, More, a tooltip, and the shortcuts dialog in both themes.
