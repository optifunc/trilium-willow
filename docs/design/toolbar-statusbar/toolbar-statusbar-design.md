...
- The action hides **both Willow toolbar and Willow status bar together**. Use pane-local presentation state `uiHidden`; collapse their layout space (`display:none` or equivalent) so the map gets the freed 71px. Hidden controls leave the tab order and accessibility tree. Do not add a floating replacement button.
...
- Toggle closes the active menu/tooltip, updates the layout, then focuses the canvas (or an existing label editor). Announce “Toolbar and status bar hidden. Use the context menu to show UI.” / “Toolbar and status bar shown.” politely. Never return focus to a now-hidden More button.
- The toggle itself does not modify the map, selection, zoom, pan, history, save state, or editing ownership. No fit/reset, note write, or undo entry is caused by it. Normal blur-to-commit behavior still applies when leaving an active label to access a menu. Preserve the production editor viewport during resize; the simulation uses its usual centered fixture layout without changing zoom.
- Default to visible in a newly opened pane. Keep the choice local to that pane for its mounted lifetime, including selection/theme/state changes. A second pane is independent. Do not serialize it into map JSON or add storage persistence. Prototype Reset demo restores both bars.
